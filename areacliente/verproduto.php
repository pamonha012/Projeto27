...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    while($tbl =  mysqli_fetch_array($resultado)){
        $cont = $tbl[0];
        if($cont == 0){
            $sql = "INSERT INTO itens_carrinho(fk_pro_id, item_quantidade, fk_cli_id, total_carrinho, numero_carrinho, carrinho_finalizado) VALUES('$idproduto','$quantidade','$idcliente','$totalparcial','$numerocarrinho','n')";
            echo"<script>window.alert('PRODUTO ADICIONADO!')</script>";
            header("Location: loja.php");
        }else{
            //verifica qual é o número do carrinho da pessoa
            ...
            while($tbl2 =  mysqli_fetch_array($resultado2)){
                ...
                $sql2 = "INSERT INTO itens_carrinho(fk_pro_id, item_quantidade, fk_cli_id, total_carrinho, numero_carrinho, carrinho_finalizado) VALUES('$idproduto','$quantidade','$idcliente','$totalparcial','$numerocarrinhocliente','n')";
...
<body>
    <div>
        <form action="verproduto.php">
            <input type="hidden" name="idproduto" value="<?=$idproduto?>" required>
            <label>Nome: </label>
            <input type="text" name="produto" value="<?=$nomeproduto?>" required disabled>
            <br>
            ...
            <br>
            <img src="data:image/jpeg;base64, <?=$imagematual?>" width="150" height="100">
            <input type="subit" value="Adionar carrinho">
        </form>
    </div>
</body>
</html>